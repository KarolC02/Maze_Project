...
void enqueue(Queue *q, Cell item) {
    ...
    if (q->front == -1) {
        q->front = q->rear = 0; // Initialize both front and rear if the queue is empty
    } else {
        q->front = (q->front - 1 + MAX_SIZE) % MAX_SIZE; // Circular decrement of front
    }
    q->items[q->front] = item; // Insert the new item at the front
}
